...
const WORKSPACES = [
  { id: 'gp', label: 'Your Practice', icon: Briefcase, subtitle: 'Your practice', active: true, path: '/dashboard/partner/group-practice', accent: '#2dd4bf', second: '#0ea5e9' },
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', flexDirection: 'column', background: '#060e1e', position: 'relative', overflow: isMobile ? 'auto' : 'hidden' }}>
      ...
      <main style={{ 
        ...
        <div style={{
          textAlign: 'center', marginBottom: isMobile ? 32 : 52, position: 'relative',
          opacity: mounted ? 1 : 0, transform: mounted ? 'translateY(0)' : 'translateY(-20px)',
          transition: 'all 0.9s cubic-bezier(0.22, 1, 0.36, 1)',
          width: isMobile ? '100%' : 'auto', paddingTop: isMobile ? 16 : 0,
        }}>
          <div style={{ position: 'absolute', top: isMobile ? -30 : -50, left: '50%', transform: 'translateX(-50%)', width: isMobile ? 280 : 360, height: isMobile ? 60 : 80, background: 'radial-gradient(ellipse, rgba(45,212,191,0.18) 0%, transparent 70%)', filter: 'blur(24px)', pointerEvents: 'none' }} />
          <h1 style={{
            fontFamily: 'Georgia, serif',
            fontSize: isMobile ? 'clamp(28px, 8vw, 40px)' : 'clamp(26px, 3.8vw, 44px)',
            fontWeight: 'normal', letterSpacing: '-0.01em',
            background: 'linear-gradient(90deg, #e2f8f5, #ffffff, #b2f0ea)',
            backgroundSize: '200% 200%',
            WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent',
            animation: 'greetShimmer 6s ease-in-out infinite', margin: 0,
            padding: isMobile ? '0 8px' : 0,
          }}>
            {getGreeting()}, {displayName}.
          </h1>
          <p style={{ fontSize: isMobile ? 11 : 13, color: '#4a6080', marginTop: isMobile ? 8 : 10, letterSpacing: '0.08em', textTransform: 'uppercase' as const }}>
        ...
        <div style={{
          ...
          alignItems: 'center', gap: isMobile ? 16 : 0, position: 'relative',
            ...
            return (
              <React.Fragment key={w.id}>
                ...
                <div
                  ...
                  style={{
                    width: isMobile ? '100%' : 240, height: isMobile ? 'auto' : 210, minHeight: isMobile ? 160 : 210,
                    borderRadius: 24, position: 'relative',
                    cursor: w.active ? 'pointer' : 'default', opacity: w.active ? 1 : 0.6,
                ...
                >
                  <div style={{
                    ...
                    filter: `blur(${isHovered ? 20 : 12}px)`, transition: 'all 0.4s ease', zIndex: 0,
                  }} />
                  <div style={{
                    position: 'relative', zIndex: 1, width: '100%', height: '100%', minHeight: isMobile ? 160 : 210,
                    borderRadius: 24,
                    ...
                    justifyContent: isMobile ? 'flex-start' : 'center',
                    gap: isMobile ? 16 : 14, overflow: 'hidden', transition: 'all 0.4s ease',
                    padding: isMobile ? '20px 24px' : 0,
                  }}>
                    <div style={{
                      position: 'absolute', top: isMobile ? '50%' : '10%', left: isMobile ? '20%' : '50%',
                    ...
                    <div style={{
                      width: isMobile ? 52 : 62, height: isMobile ? 52 : 62, borderRadius: isMobile ? 14 : 18,
                      background: `linear-gradient(135deg, ${w.accent}30, ${w.second}18)`,
                      border: `1px solid ${w.accent}66`,
                      display: 'flex', alignItems: 'center', justifyContent: 'center',
                      boxShadow: `0 0 24px ${w.accent}44`, position: 'relative', zIndex: 1, flexShrink: 0,
                    }}>
                    ...
                    {!w.active && (
                      <div style={{
                        position: isMobile ? 'relative' : 'absolute', bottom: isMobile ? 'auto' : 16,
                        padding: '3px 14px', borderRadius: 20,
                        background: `${w.accent}18`, border: `1px solid ${w.accent}33`,
                        color: w.accent, fontSize: 11, letterSpacing: '0.06em', flexShrink: 0,
                      }}>Coming Soon</div>
                    )}
                  </div>
                </div>
